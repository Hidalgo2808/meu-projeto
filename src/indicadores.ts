...
export function rankingDoMes(
  ...
  const porColab = new Map<string, { faltas: number; subs: number; motivos: Record<string, number>; ultima: string }>();
  ...
  for (const [data, dia] of Object.entries(registros)) {
    ...
    for (const [colabId, reg] of Object.entries(dia)) {
      ...
      const acc = porColab.get(colabId) ?? { faltas: 0, subs: 0, motivos: {}, ultima: '' };
      if (reg.situacao === 'falta') acc.faltas += 1;
      else acc.subs += 1;
      const mot = normalizaMotivo(reg.motivo);
  ...
  for (const [colabId, acc] of porColab) {
    ...
    const total = acc.faltas + acc.subs;
...
export function motivosDoMes(registros: BancoRegistros, mesKey: string): Array<{ motivo: string; qtd: number }> {
  ...
  for (const [data, dia] of Object.entries(registros)) {
    ...
    for (const reg of Object.values(dia)) {
      if (reg.situacao === 'presente') continue;
      const m = normalizaMotivo(reg.motivo);
...
export function evolucaoMensalColaborador(registros: BancoRegistros, colabId: string): Array<{ mesKey: string; faltas: number }> {
  ...
  return meses.map((mesKey) => {
    ...
    return { mesKey, faltas };
  });
}

export function exportarRankingCSV(mesKey: string, ranking: RankingMensalItem[]): void {
  const header = 'Posição;Colaborador;Matrícula;Função;Faltas;Substituições;Total ausências;Taxa de falta (%);Motivo principal;Última falta\n';
  const body = ranking
    .map(
      (r, i) =>
        `${i + 1};${r.nome};${r.matricula};${r.funcao};${r.faltas};${r.substituicoes};${r.totalAusencias};${String(r.taxaFalta).replace('.', ',')};${r.principalMotivo};${r.ultimaFalta}`,
    )
    .join('\n');
